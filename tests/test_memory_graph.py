...
def test_semantic_search(db):
    ...
    # Add some sample memories with embeddings (in a real app, these would be generated by an ML model)
    samples = [
        ("The sky is blue", [0.1, 0.2, 0.3]),
        ("The ocean is deep", [0.15, 0.25, 0.35]),
        ("Apples are red", [0.8, 0.9, 0.1])
    ]
    
    for content, embedding in samples:
        db.create_memory(content, {"embedding": embedding})
    
    # Search for something similar to the first sample
    query_embedding = [0.12, 0.22, 0.32]  # Similar to the first sample
